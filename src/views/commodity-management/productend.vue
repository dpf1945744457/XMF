<template>
  <div style="background:#fff;padding: 18px">
    ...
    >
      ...
      <el-form-item label="商品主图" prop="gallerys">
        <avatarUpload :url="ruleForm.picUrl" @change="changeImgs($event,'picUrl')"></avatarUpload>
      </el-form-item>

      <el-form-item label="商品画廊" prop="gallerys">
        <imgVideoUpload :typeInfo="1" :data="galleryList" @change="changeGallery"></imgVideoUpload>
        <span>注：最多上传10张</span>
      </el-form-item>
      ...
      <div style="padding-left: 30px;">
        ...
        >
          ...
          <el-table-column prop="counterPrice" align="center" label="专柜价格">
            <template slot-scope="scope">
              <el-input v-model="scope.row.counterPrice"></el-input>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="customsUnitNum" align="center" label="清关规格">
            <template slot-scope="scope">
              <el-input v-model="scope.row.customsUnitNum" type="number">
          ...
          <el-table-column prop="postage" align="center" label="邮费">
            <template slot-scope="scope">
              <el-input v-model="scope.row.postage"></el-input>
            </template>
          </el-table-column>
    ...
    <el-dialog :title="rulltitle" :visible.sync="dialogFormVisible" width="450px">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" v-if="rulltitle=='编辑规格'" @click="endrul('gzform')">确 定</el-button>
      </div>
    </el-dialog>
    <!-- 规格设置 -->
    ...
    <el-dialog title="修改规格" :visible.sync="productVisiable" width="960px">
      <el-form
        ...
        label-width="110px"
      >
        <el-form-item label="规格选项" prop="specifications">
          <span
            v-for="(text,index) in productForm.name"
            :key="index"
          >{{text}}:{{productForm.specifications[index]}}</span>
        </el-form-item>
        <!--<el-form-item prop="goodsSn" label="商品货号">-->
        <!--<el-input v-model="productForm.goodsSn"></el-input>-->
        <!--</el-form-item>-->
        <el-form-item label="专柜价格" prop="counterPrice">
          <el-input v-model="productForm.counterPrice"></el-input>
        </el-form-item>
        ...
        <el-form-item prop="customsUnitNum" label="清关规格">
          <el-input v-model="productForm.customsUnitNum" type="number">
            <template slot="append">{{danwei}}</template>
          </el-input>
        </el-form-item>
        <el-form-item label="供货价" prop="supplyPrice">
          <el-input v-model="productForm.supplyPrice" type="number"></el-input>
        </el-form-item>
        <el-form-item label="商品条码">
          <el-input v-model="productForm.barCode"></el-input>
        </el-form-item>
        <el-form-item prop="netWeight" label="商品净重">
          <el-input v-model="productForm.netWeight" type="number">
            <template slot="append">kg</template>
          </el-input>
        </el-form-item>
        <el-form-item prop="grossWeight" label="商品毛重">
          <el-input v-model="productForm.grossWeight" type="number">
            <template slot="append">kg</template>
          </el-input>
        </el-form-item>
        <el-form-item label="邮费" prop="postage">
          <el-input v-model="productForm.postage" type="number"></el-input>
        </el-form-item>
        ...
        <el-form-item label="是否包税" prop="taxFlag">
          <el-switch
            ...
            inactive-text="否"
          ></el-switch>
          <span>税费：{{productForm.incomeTax}}</span>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
import { goodsid, goodsend } from "@/api/goods";
...
export default {
  ...
  methods: {
    init() {
      goodsid({ goodsId: this.$route.query.id }).then((response) => {
        this.ruleForm = response.data;
        let gallerys = [];
        for (let i = 0; i < this.ruleForm.gallery.length; i++) {
          const gallery = this.ruleForm.gallery[i];
          let list = { image: gallery };
          gallerys.push(list);
          this.galleryList.push(list);
        }
        this.ruleForm.gallerys = gallerys;
        this.ruleForm.shipmentRegion = JSON.stringify(
          this.ruleForm.shipmentRegion
        );
        this.tableData = this.ruleForm.goodsDetailProducts;
        let rulllist = [];
        let jmap = {};

        this.ruleForm.goodsDetailSpecifications.forEach(function (al) {
          var key = al.specification;
        ...
        for (var i = 0; i < keys.length; i++) {
          ...
          rulllist.push({ name: rs[0], guize: jmap[keys[i]] });
        }
        this.rulllist = rulllist;
        let name = [];
        ...
        this.listguize = this.tableData.length;
        this.canshutableData = this.ruleForm.goodsDetailAttributes;
      });
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.ruleForm.goodsDetailAttributes = this.canshutableData;
          console.log(this.ruleForm);
          goodsend(this.ruleForm).then((response) => {
            this.$notify({
              title: "成功",
              message: "更新成功",
              type: "success",
      ...
      this.$confirm("此操作将重新组合下列数据, 是否继续?", "提示", {
        ...
        .then(() => {
          if (this.rulllist.length != 1) {
            ...
            if (this.rulllist[0].guize.length > this.listguize) {
              let namelist = [];
              for (let index = 0; index < this.rulllist.length; index++) {
                const element = this.rulllist[index];
                namelist.push(element.name);
              }
              console.log(this.rulllist.length);
              let i = this.rulllist.length - 1;
              console.log(this.rulllist[i]);
              let data = {
                barCode: "",
                counterPrice: "",
                customsUnitNum: "",
                ...
                supplyPrice: "",
                specifications: [
                  this.rulllist[0].guize[this.rulllist[0].guize.length - 1]
                    .value,
                ],
                specifications1: this.rulllist[0].guize[
                  this.rulllist[0].guize.length - 1
                ].value,
                name: namelist,
                taxFlag: false,
                url: "",
              };
              this.listguize = this.rulllist[0].guize.length;
              this.tableData.push(data);
            } else {
              this.tableData = [];
              let lits = [];
